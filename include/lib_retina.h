...
#ifdef __GNUC__
#define RTNA_ARM_PowerDown(ctl) asm("MCR p15, 0, %[value], c7, c0, 4" : : [value]"r"(ctl));
#else
__inline void RTNA_ARM_PowerDown(unsigned int ctl)
{
 ...
 * 
 */
void	RTNA_AIC_Open(AITPS_AIC pAIC, MMP_ULONG id, IntHandler pfHandler, MMP_ULONG type);	// __attribute__((always_inline));
void	RTNA_AIC_IRQ_En(AITPS_AIC pAIC, MMP_ULONG id);	// __attribute__((always_inline));
void	RTNA_AIC_IRQ_Dis(AITPS_AIC pAIC, MMP_ULONG id); // __attribute__((always_inline));
void	RTNA_AIC_IRQ_DisAll(AITPS_AIC pAIC);			// __attribute__((always_inline));
void	RTNA_AIC_IRQ_ClearAll(AITPS_AIC pAIC);			// __attribute__((always_inline));
void	RTNA_AIC_IRQ_EOI(AITPS_AIC pAIC);				// __attribute__((always_inline));
...
// #define MMP_PRINT_RET_ERROR_MACRO(_FUNC, _LINE, level, ret_val, description) RTNA_DBG_Str(level, FG_RED(_FUNC",Line:")); 
#define MMP_PRINT_RET_ERROR_MACRO(_LINE, level, ret_val, description) RTNA_DBG_Str(level, FG_RED("Line:")); \
        RTNA_DBG_Dec(level, _LINE); \
        RTNA_DBG_Str(level, FG_RED(",Err:")); \
        RTNA_DBG_Long(level, ret_val); \
        RTNA_DBG_Str(level, FG_RED("\r\nDesc: "description"\r\n"))             

// #define MMP_PRINT_RET_ERROR(level, ret_val, description) MMP_PRINT_RET_ERROR_MACRO(__func__, __LINE__, level, ret_val, description)
#define MMP_PRINT_RET_ERROR(level, ret_val, description) MMP_PRINT_RET_ERROR_MACRO( __LINE__, level, ret_val, description)
