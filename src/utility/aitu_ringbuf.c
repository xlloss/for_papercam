...
*/
int AUTL_RingBuf_Init(AUTL_RINGBUF *ring, MMP_ULONG buf, MMP_ULONG size)
{
    if (!ring)
        return RINGBUF_ERR_NULL_HANDLE;

    ring->buf       = buf;
    ring->buf_phy   = (void *)buf;
    ring->size      = size;
...
//------------------------------------------------------------------------------
//  Function    : AUTL_RingBuf_Flush
//------------------------------------------------------------------------------
/** 
    @brief  Flush all data in the buffer.
    @param[in] ring : ring buffer handler
    @return 0 for success, others for failed.
*/
int AUTL_RingBuf_Flush(AUTL_RINGBUF *ring)
{
    OS_CRITICAL_INIT();
    ...
    OS_ENTER_CRITICAL();
    ring->ptr.rd = ring->ptr.wr;
    ring->ptr.rd_wrap = ring->ptr.wr_wrap = 0;
    OS_EXIT_CRITICAL();
