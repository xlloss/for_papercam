...
//  Mode0	Mode1   Mode2	Mode3	Mode4
    ...
    {1920,  2048,   1152,	1536,	1536},	// usBayerOutW
    {1080,  1536,	648,	1536,	1536},	// usBayerOutH
    #endif
    #endif
    ...
    #else
    {1920,  2048, 	800,	1536,	1536},	// usScalInputW
    {1080,  1536, 	600,	1536,	1536},	// usScalInputH
    #endif
...
const ISP_UINT8 Sensor_IQ_CompressedText[] = 
{
  ...
  #include "isp_8428_iq_data_v3_AR0330_ezmode_20160823_Diane.xls.ciq.txt"
#else
...
#if (VRP_DEMO_EN)
    0x301A, 0x0059,
    // Power up sequence on Spec
	SENSOR_DELAY_REG, 7,	// Wait 150,000 EXTCLK periods, 150K / 24MHz = 6.25 ms
	0x3052, 0xA114,		    // Configure the internal register initialization process
	0x304A, 0x0070,		    // Start the internal register initialization process
	SENSOR_DELAY_REG, 2,    // Wait 150,000 EXTCLK periods, 150K / 24MHz = 6.25 ms
...
ISP_UINT16 SNR_AR0330_Reg_1544x1540_30FPS[] =
{
    //SENSOR_DELAY_REG, 	100
};

ISP_UINT16 SNR_AR0330_Reg_1544x1540_15FPS[] =
{
    //SENSOR_DELAY_REG, 	100
};
...
static void SNR_Cust_InitConfig(void)
{
	...
	RTNA_DBG_Str(0, "SNR_Cust_InitConfig AR0330\r\n");
    ...
    SensorCustFunc.OprTable->usSize[RES_IDX_1544x1540_30FPS]  	= (sizeof(SNR_AR0330_Reg_1544x1540_30FPS)/sizeof(SNR_AR0330_Reg_1544x1540_30FPS[0]))/2;
    SensorCustFunc.OprTable->uspTable[RES_IDX_1544x1540_30FPS]	= &SNR_AR0330_Reg_1544x1540_30FPS[0];
    SensorCustFunc.OprTable->usSize[RES_IDX_1544x1540_15FPS]  	= (sizeof(SNR_AR0330_Reg_1544x1540_15FPS)/sizeof(SNR_AR0330_Reg_1544x1540_15FPS[0]))/2;
    SensorCustFunc.OprTable->uspTable[RES_IDX_1544x1540_15FPS]	= &SNR_AR0330_Reg_1544x1540_15FPS[0];
    ...
    SensorCustFunc.VifSetting->powerOffSetting.ubEnPinDelay           = 5;
    ...
    ulSot = (0x1F * ulFreq) / 400000; // SOT 0x1F is for VIF 400MHz
#endif
...
void SNR_Cust_SetFlip(MMP_UBYTE ubSnrSel, MMP_UBYTE ubMode)
{
    ...
    case MMPF_SENSOR_NO_FLIP:
        gsSensorFunction->MMPF_Sensor_SetReg(ubSnrSel, 0x3040, 0x0000);// [15]: flip, [14]: mirror
    	break;
    case MMPF_SENSOR_COLUMN_FLIP:
        gsSensorFunction->MMPF_Sensor_SetReg(ubSnrSel, 0x3040, 0x8000);// [15]: flip, [14]: mirror    
        break;
    case MMPF_SENSOR_ROW_FLIP:
        gsSensorFunction->MMPF_Sensor_SetReg(ubSnrSel, 0x3040, 0x4000);// [15]: flip, [14]: mirror
        break;
    case MMPF_SENSOR_COLROW_FLIP:
        gsSensorFunction->MMPF_Sensor_SetReg(ubSnrSel, 0x3040, 0xc000);// [15]: flip, [14]: mirror
        break;
    default:
		break;
    }
