...
MMP_ERR MMPD_RAWPROC_SetStoreBuf(MMP_UBYTE ubRawId, MMP_RAW_STORE_BUF *pRawBuf)
{
    ...
    for (i = 0; i < pRawBuf->ulRawBufCnt; i++) {
    	...
    	printc("RawId %d BufId %d Addr %x\r\n",ubRawId, i, pRawBuf->ulRawBufAddr[i]);
    }
...
MMP_ERR MMPD_RAWPROC_EnableRingStore(MMP_UBYTE ubRawId, MMP_UBYTE ubPlane, MMP_BOOL bRingBufEn)
{
	...
	MMPF_RAWPROC_EnableRingStore(ubRawId, ubPlane, bRingBufEn);
...
//  Description :
//------------------------------------------------------------------------------
MMP_ERR MMPD_RAWPROC_SetDeInterlaceBuf(MMP_ULONG ulYAddr, MMP_ULONG ulUAddr, MMP_ULONG ulVAddr)
{
    return MMPF_RAWPROC_SetDeInterlaceBuf(ulYAddr, ulUAddr, ulVAddr);
}
