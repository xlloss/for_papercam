...
MMP_USHORT MMPF_SD_CheckCardWP(stSDMMCHandler *SDMMCArg){return 1;}
...
void MMPF_SD_InitHandler(void)
{
    ...
    /* for WIFI SDIO PADSET 2*/
    m_SDMMCArg[0].ubSdPadMapping = MMPF_SD0_PAD0; // set later by compiling option
    m_SDMMCArg[1].ubSdPadMapping = SD1_PAD_NUM;
    m_SDMMCArg[2].ubSdPadMapping = MMPF_SD2_PAD0;
...
void MMPF_SD_ISR(void)
{
    ...
    do {
        if (MMPF_SYS_CheckClockEnable(MMPF_SYS_CLK_SD(tIdx))) {
...
MMP_ERR  MMPF_SD_WriteSector(stSDMMCHandler *SDMMCArg, MMP_ULONG dmastartaddr, MMP_ULONG startsect, MMP_USHORT sectcount)
{
	...
	while(1) {
		if (pSD->SD_CPU_INT_SR & CMD_SEND_DONE)
			break;
		else {
		    MMPF_SYS_WaitUs(1);
			ulUScount++;
		    if ((ulUScount/1000) > SD_CMD_SENDDONE_TIMEOUT) {
				MMP_PRINT_RET_ERROR(SD_MMC_CLK_DBG_MSG, 0, "SD WriteSector Timeout\r\n");
				return MMP_SD_ERR_TIMEOUT;
		    }
		}
		ulUScount++;
	}
...
MMP_ERR  MMPF_SD_ReadPhysicalSector(stSDMMCHandler *SDMMCArg, MMP_ULONG dmastartaddr, MMP_ULONG startsect, MMP_USHORT sectcount) 
{
	...
	while(1) {
		if (pSD->SD_CPU_INT_SR & CMD_SEND_DONE)
			...
			ulUScount++;
		    if ((ulUScount/1000) > SD_CMD_SENDDONE_TIMEOUT) {
				MMP_PRINT_RET_ERROR(SD_MMC_CLK_DBG_MSG, 0, "SD ReadPhysicalSector Timeout\r\n");
				return MMP_SD_ERR_TIMEOUT;
		    }
		}
		ulUScount++;
...
MMP_ERR  MMPF_SD_ReadSector(stSDMMCHandler *SDMMCArg, MMP_ULONG dmastartaddr, MMP_ULONG startsect, MMP_USHORT sectcount)
{
    ...
    while(1) {
        ...
        for (i = 0; i < m_bReadRetryCnt[SDMMCArg->id]; i++) {
            err = MMPF_SD_ReadPhysicalSector(SDMMCArg, dmastartaddr, startsect, sectcount);
            if (err == MMP_ERR_NONE)
                break;
			else
				return err;
...
{
    if (bEnable) {
        MMPF_SYS_EnableClock(MMPF_SYS_CLK_SD0 + SDMMCArg->id, MMP_TRUE);
    }
    else {
        MMPF_SYS_EnableClock(MMPF_SYS_CLK_SD0 + SDMMCArg->id, MMP_FALSE);
    }
}
#endif
...
MMP_ERR MMPF_SD_ConfigPadMapping(stSDMMCHandler *SDMMCArg, MMPF_SD_PAD padid, MMP_BOOL b8bitBus)
{
    ...
    if ((SDMMCArg->id == MMPF_SD_0) && (padid == MMPF_SD0_PAD0)) {
        ...
        pPAD->PAD_IO_CFG_PCGPIO[0] = 0x23; //SD_CLK 3.3V 14.4mA
        pPAD->PAD_IO_CFG_PCGPIO[1] = 0x25; //SD_CMD 3.3V 14.4mA, pull-up
        pPAD->PAD_IO_CFG_PCGPIO[2] = 0x05; //SD_DAT0 3.3V 12mA, pull-up
        pPAD->PAD_IO_CFG_PCGPIO[3] = 0x05; //SD_DAT1 3.3V 12mA, pull-up
        pPAD->PAD_IO_CFG_PCGPIO[4] = 0x05; //SD_DAT2 3.3V 12mA, pull-up
        pPAD->PAD_IO_CFG_PCGPIO[5] = 0x05; //SD_DAT3 3.3V 12mA, pull-up
    }
    else if ((SDMMCArg->id == MMPF_SD_2) && (padid == MMPF_SD2_PAD0)) {
        pGBL->GBL_SD_PAD_CFG |= GBL_SD2_PAD_EN;
        if (b8bitBus == MMP_TRUE)
            bPadCfgCorrect = MMP_FALSE;
        pPAD->PAD_IO_CFG_PCGPIO[12] = 0x23; //SD_CLK 3.3V 14.4mA
        pPAD->PAD_IO_CFG_PCGPIO[13] = 0x25; //SD_CMD 3.3V 14.4mA, pull-up
        pPAD->PAD_IO_CFG_PCGPIO[14] = 0x05; //SD_DAT0 3.3V 12mA, pull-up
        pPAD->PAD_IO_CFG_PCGPIO[15] = 0x05; //SD_DAT1 3.3V 12mA, pull-up
        pPAD->PAD_IO_CFG_PCGPIO[16] = 0x05; //SD_DAT2 3.3V 12mA, pull-up
        pPAD->PAD_IO_CFG_PCGPIO[17] = 0x05; //SD_DAT3 3.3V 12mA, pull-up
    }
    else if (SDMMCArg->id == MMPF_SD_1) {
        pGBL->GBL_SD_PAD_CFG &= ~(GBL_SD1_PAD_MASK);
        pGBL->GBL_SD_PAD_CFG |= GBL_SD1_PAD(SD1_PAD_NUM - MMPF_SD1_PAD0);
        if (SD1_PAD_NUM == MMPF_SD1_PAD0) {
            pPAD->PAD_IO_CFG_PCGPIO[6] = 0x23; //SD_CLK 3.3V 14.4mA
            pPAD->PAD_IO_CFG_PCGPIO[7] = 0x25; //SD_CMD 3.3V 14.4mA, pull-up
            pPAD->PAD_IO_CFG_PCGPIO[8] = 0x05; //SD_DAT0 3.3V 12mA, pull-up
            pPAD->PAD_IO_CFG_PCGPIO[9] = 0x05; //SD_DAT1 3.3V 12mA, pull-up
            pPAD->PAD_IO_CFG_PCGPIO[10] = 0x05; //SD_DAT2 3.3V 12mA, pull-up
            pPAD->PAD_IO_CFG_PCGPIO[11] = 0x05; //SD_DAT3 3.3V 12mA, pull-up
        }
        else if (SD1_PAD_NUM == MMPF_SD1_PAD1) {
            pPAD->PAD_IO_CFG_PBGPIO2[0] = 0x23; //SD_CLK 3.3V 14.4mA
            pPAD->PAD_IO_CFG_PBGPIO2[1] = 0x25; //SD_CMD 3.3V 14.4mA, pull-up
            pPAD->PAD_IO_CFG_PBGPIO2[2] = 0x05; //SD_DAT0 3.3V 12mA, pull-up
            pPAD->PAD_IO_CFG_PBGPIO2[3] = 0x05; //SD_DAT1 3.3V 12mA, pull-up
            pPAD->PAD_IO_CFG_PBGPIO2[4] = 0x05; //SD_DAT2 3.3V 12mA, pull-up
            pPAD->PAD_IO_CFG_PBGPIO2[5] = 0x05; //SD_DAT3 3.3V 12mA, pull-up
            if (b8bitBus) {
                pPAD->PAD_IO_CFG_PBGPIO[12] = 0x05; //SD_DAT4 3.3V 12mA, pull-up
                pPAD->PAD_IO_CFG_PBGPIO[13] = 0x05; //SD_DAT5 3.3V 12mA, pull-up
                pPAD->PAD_IO_CFG_PBGPIO[14] = 0x05; //SD_DAT6 3.3V 12mA, pull-up
                pPAD->PAD_IO_CFG_PBGPIO[15] = 0x05; //SD_DAT7 3.3V 12mA, pull-up
            }
        }
        if ((b8bitBus == MMP_TRUE) && (SD1_PAD_NUM != MMPF_SD1_PAD1))
            bPadCfgCorrect = MMP_FALSE;
    }
    else {
        bPadCfgCorrect = MMP_FALSE;
...
MMP_ERR  MMPF_SD_InitialInterface(stSDMMCHandler *SDMMCArg)
{
    ...
    MMPF_PLL_GetGroupFreq(CLK_GRP_GBL, &m_ulSDG0SrcClk);
    #if (CHIP == P_V2)
    ...
    pGBL->GBL_CLK_0_LCD_SD_DIV = ((pGBL->GBL_CLK_0_LCD_SD_DIV)&0x0F) | (GBL_CLK_SD_DIV(divValue));
    #endif

    // Enable SD module clock before access SD module's register
    #if (CHIP == P_V2)
    MMPF_SYS_EnableClock(MMPF_SYS_CLK_SD, MMP_TRUE);
    #endif
    #if (CHIP == MCR_V2)
    MMPF_SYS_EnableClock(MMPF_SYS_CLK_SD0 + SDMMCArg->id, MMP_TRUE);
    #endif
...
MMP_ERR  MMPF_MMC_Reset(stSDMMCHandler *SDMMCArg)
{
    ...
    //pGBL->GBL_CLK_0_LCD_SD_DIV = ((pGBL->GBL_CLK_0_LCD_SD_DIV)&0x0F) | (GBL_CLK_SD_DIV(1));
    ...
    do {
        ...
        // CMD  1
        if((!MMPF_SD_SendCommand(SDMMCArg, SEND_OP_COND, MMC_SUPPORT_VOLTAGE))) {
    ...
    //MMPF_PLL_GetGroupFreq(CLK_GRP_GBL, &ulMMCclk);
...
MMP_ERR  MMPF_SD_Reset(stSDMMCHandler *SDMMCArg)
{
    ...
    do {
        if(MMPF_SD_SendCommand(SDMMCArg, SEND_IF_COND, (0x1 << 8)| 0xAA)) {
            ...
            sd2 = 0;   // for HCS index, (0)for Ver1.x, (1)for Version at least 2.0
        }
        else{
            RTNA_DBG_Str3("sd2.0\r\n");
            sd2 = 1;
    ...
    do {
        ...
        if(!MMPF_SD_SendCommand(SDMMCArg, APP_CMD, 0)) {
            if(!MMPF_SD_SendCommand(SDMMCArg, SD_APP_OP_COND, SD_SUPPORT_VOLTAGE | (sd2<<30))) {
                i++;
            }
            else {
                #if (ENABLE_SD_CLKGATING == 1)
    ...
    /* <Identification mode> if the RCA is Zero, continue CMD3 for 3 times
     ulCardAddr would be like this 0x12340500->0x12350700->12360700->1237xxxx
       don't care for last 2 Bytes; the first 2 Bytes is increasing for  RCA     
    */
    i = 0;
    do{
        // CMD  3 
        ...
        i++;
    } while( (((SDMMCArg->ulCardAddr)&(0xFFFF0000)) == 0 ) && (i<5) ); 
    i = 0;    
    if( ( (SDMMCArg->ulCardAddr)&(0xFFFF0000) ) == 0 ) 
        return MMP_SD_ERR_RESET;   
    /*
    // CMD  3
    if(!MMPF_SD_SendCommand(SDMMCArg, SEND_RELATIVE_ADDR, 0)) {
    ...
    */
    ...
    //#endif
        {
            //MMPF_PLL_GetGroupFreq(CLK_GRP_GBL, &ulSDclk);
...
MMP_ERR  MMPF_SD_GetCardInfo(stSDMMCHandler *SDMMCArg, MMPF_SD_CARDTYPE btype)
{
    ...
    if (!MMPF_SD_SendCommand(SDMMCArg, SEND_CSD, SDMMCArg->ulCardAddr)) {
        ...
        // For 64GB, (pSD->SD_RESP.W[4] & 0x3F) should be 1.
...
MMP_ERR  MMPF_SD_SendCommand(stSDMMCHandler *SDMMCArg, MMP_UBYTE command, MMP_ULONG argument)
{
    AITPS_SD    pSD = SDMMCArg->pSD;
    MMP_USHORT  tmp;
	MMP_ULONG   ulStampStart,ulStampStop; 
    MMP_ULONG   ulTempSum = 0; 
    MMP_ULONG   ulORGDATATOUT;             
    ...
    if (command == GO_IDLE_STATE) {
        //while (!(pSD->SD_CPU_INT_SR & CMD_SEND_DONE));        
        ulStampStart = OSTimeGet();         
               
        while(1) {   
            if (pSD->SD_CPU_INT_SR & CMD_SEND_DONE)
                break;
            else {
                ulStampStop = OSTimeGet();
                if (ulStampStop >= ulStampStart)
                    ulTempSum = (ulStampStop - ulStampStart);
                else //overflow of ulStampStop
                    ulTempSum = (0xFFFFFFFF - ulStampStart + ulStampStop);
                
                if (ulTempSum > SD_RESPONSE_BUSY_TIMEOUT) {
                    RTNA_DBG_Str(0, "GO_IDLE_STATE CMD_SEND_DONE Timeout.\r\n");
					return MMP_SD_ERR_COMMAND_SENDDONETIMEOUT;
                }            
            }    
        }        
        ...
        //SD spec, busy time for read should not over 100ms , write should not over 250ms
        //SD CLK is 528MHz/2/1024 = 264KHz
        //SD_DATA_TOUT*256*SDCLK > 250ms ==> SD_DATA_TOUT at least 0x100,rounds to 0x300
	    ulORGDATATOUT = pSD->SD_DATA_TOUT;
	    pSD->SD_DATA_TOUT = 0x300; 
        ...
        while(1) {                    
            ...
            if ((tmp & CMD_SEND_DONE) && (!(tmp & CMD_RESP_TOUT)) && (!(tmp & BUSY_TOUT))) {
                pSD->SD_DATA_TOUT = ulORGDATATOUT;
                break;
            }
            ...
            if(tmp & BUSY_TOUT) {
                #if 0
                RTNA_DBG_Str0("AITPS_SD for BUSY_TOUT\r\n");
                #endif
                pSD->SD_DATA_TOUT = ulORGDATATOUT;
                return MMP_SD_ERR_COMMAND_FAILED;
            }
            ...
            if (ulTempSum > SD_RESPONSE_BUSY_TIMEOUT) {
                RTNA_DBG_Str(0, "MMPF_SD_SendCommand over SD_RESPONSE_BUSY_TIMEOUT\r\n");
                pSD->SD_DATA_TOUT = ulORGDATATOUT;
                return MMP_SD_ERR_COMMAND_FAILED;
            }
            */
    ...
    if ((command == STOP_TRANSMISSION)||(command == SELECT_CARD)||(command == VENDER_COMMAND_62)) { /* ac, R1b */
        ...
        if (pSD->SD_RESP.D[3] & (COM_CRC_ERROR | ILLEGAL_COMMAND
            ...
            #if (SDIO_WRITE_BLOCK_RECORVERY == 1)
            return MMP_SD_ERR_COMMAND_FAILED;
            #endif
        }
    }
...
//------------------------------------------------------------------------------
//  Function    : MMPF_SDIO_ReadMultiRegBlock
//------------------------------------------------------------------------------
static MMP_ERR MMPF_SDIO_ReadMultiRegBlock(
            stSDMMCHandler      *SDMMCArg,
            MMP_UBYTE           fun_num,
            MMP_UBYTE           blk_mode,
            MMP_UBYTE           op_code,
            ...
            MMP_ULONG           dst_addr)
{
    ...
    // Clear all previous status
    pSD->SD_CPU_INT_SR = (DATA_CRC_ERR | DATA_TOUT | DATA_ST_BIT_ERR |
                          DATA_SEND_DONE | CMD_RESP_CRC_ERR | CMD_RESP_TOUT |
                          BUSY_TOUT | CMD_SEND_DONE);

#if (SDIO_CPU_NONBLOCKING==1)
    //++Philip@070531: Add for ASYNC SDIO READ
    // Enable interrupt when "DATA SEND DONE"
    pSD->SD_CPU_INT_EN |= DATA_SEND_DONE;
#endif /* (SDIO_CPU_NONBLOCKING==1) */

    ret = MMPF_SD_SendCommand(SDMMCArg, IO_RW_EXTENDED,
                              (RW_FLAG(0) | FUN_NUM(fun_num) |
                              BLK_MODE(blk_mode) | OP_CODE(op_code) |
                              REG_ADD(reg_addr) | count));
...
MMP_ERR   MMPF_SDIO_ReadMultiReg(
            ...
            MMP_UBYTE          *p_dst)
{
    #if (SDIO_READ_BLOCK_RECORVERY == 1)
    MMP_ULONG 	ulTickCount1,ulTickCount2;

    ulTickCount1 = OSTimeGet();
    #endif

    MMPF_SDIO_ReadMultiRegBlock(SDMMCArg, fun_num, blk_mode, op_code,
                                reg_addr, count, blk_size, (MMP_ULONG)p_dst);

#if (SDIO_READ_BLOCK_RECORVERY == 1)
	ulTickCount2 = OSTimeGet();
	if((ulTickCount2 - ulTickCount1) > 100){
		return MMP_SD_ERR_COMMAND_FAILED;
    }
#endif
    return MMP_ERR_NONE;
