...
#else
#include "isp_8428_iq_data_v2_ps1210_JCLab_20151013_Fisher.xls.ciq.txt"
#endif
};
...
#if 1
//Fisher
//Gain = power(2, value / 16)
const ISP_UINT16 PS1210_GainTable[112] = 
{
	256,	//	0	1X
	267,	//	1
	279,	//	2
...
static void SNR_Cust_InitConfig(void)
{
    ...
    // Since PS1210 requests special sensor clock, 27 MHz (instead of 24 MHz), please make sure: #define CUSTOMIZED_DPLL1_CLOCK (1) 
	SensorCustFunc.VifSetting->clockAttr.ubClkFreqDiv 				= ulSensorClkSrc / ulSensorMCLK;
...
/*ENG*/						,0x2FFFFFF, 1817640*16,	1817640*8,	1817640*4,	1817640*2,  1817640,	889020,		396000,		190740,		95370,	44975,	22274,	11528,	5808,	2660,	1144,	512,   256
/*Tar*/						//,43,		55,		 	64,	        75,			82,	 		94,	 		110,	 	122,	    135,	    143,	155,	170,	185,	195,	220,	240,	250,   270
/*Tar*/						,43,		48,			53,			61,			72,			93,			128,		165,		190,		207,	230,	238,	244,	250,	256,	256,	256,	256
 };	
...
void SNR_Cust_SetShutter(MMP_UBYTE ubSnrSel, MMP_ULONG shutter, MMP_ULONG vsync)
{
	...
	// shutter
	gsSensorFunction->MMPF_Sensor_SetReg(ubSnrSel, 0x03, 0x01);//bank GroupB
	gsSensorFunction->MMPF_Sensor_SetReg(ubSnrSel, 0xC0, (new_shutter >> 8));
	gsSensorFunction->MMPF_Sensor_SetReg(ubSnrSel, 0xC1, new_shutter);
