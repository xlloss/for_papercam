...
static MMP_ULONG        m_ulSensorMemAddr = 0;
...
/*static */MMP_ERR MMPS_Sensor_InitPresetMode(MMP_UBYTE ubSnrSel, MMP_UBYTE ubPreviewMode)
{
    ...
    // 2017_07_17 , don't put here , since sensor is not initaized
...
static MMP_ERR MMPS_Sensor_SetMode(MMP_UBYTE ubSnrSel, MMP_SNR_MODE sSnrMode)
{
    ...
    if (m_ulSensorMemAddr == 0) 
    {
        MMPD_Sensor_GetHWBufferSize(&ulSize);
        ulSize = ALIGN32(ulSize);
        m_ulSensorMemAddr = MMPF_SYS_HeapMalloc(SYS_HEAP_DRAM, ulSize, 32);
        m_ulSensorMemEnd  = m_ulSensorMemAddr + ulSize;
	    MMPD_Sensor_AllocateBuffer(m_ulSensorMemAddr, &ulSize);
	}
...
//------------------------------------------------------------------------------
//  Function    : MMPS_Sensor_GetPreviewMode
//  Description :
//------------------------------------------------------------------------------
/** @brief Get sensor current preview mode

The function can be called after sensor has been set the preview mode.

@return It reports the sensor current preview mode
*/
MMP_ERR MMPS_Sensor_GetPreviewMode(MMP_UBYTE ubSnrSel)
{
    return m_ubCurPreviewMode[ubSnrSel];
}
